...
function defineSlidesMasters(pres, theme, brand) {
  ...
  if (isQuarks) {
    const websiteText = brand?.website ? `www.${brand.website.replace(/^https?:\/\/(www\.)?/, '')}` : 'www.qtsolv.com';
    lightObjects.push({
      text: {
        text: websiteText,
        options: {
          x: slideW - 3.5, y: footerLogoY, w: 2.2, h: 0.2,
...
async function renderCover(slide, spec, pres) {
  ...
  // Main title — dynamic font size based on title length and box dimensions
  ...
  // Subtitle — dynamic font size to fit full text
  const coverSubFont = fonts(spec).body;
  const coverSubSize = fitFontSize(spec.subtitle, 6.0, 0.5, {
    fontName: coverSubFont, minSize: 10, maxSize: 16, fillRatio: 0.90
  });
  slide.addText(spec.subtitle, {
    x: 0.5, y: 2.85, w: 6.0, h: 0.5,
    fontSize: coverSubSize,
    fontFace: coverSubFont,            // Calibri Light for quarks
    color: t.body_dark,
    align: 'left', margin: 0
  });
...
async function renderSectionHeader(slide, spec, pres) {
  ...
  // ── Main section title — dynamic font size ───────────────────────
  const sectionTitle = isQuarks
    ? (spec.title || 'Section').toUpperCase()  // spec: ALL CAPS on dark bg
    : (spec.title || 'Section');
  const sectionTitleW = isQuarks ? 8.5 : 7;
  const sectionTitleH = isQuarks ? 1.8 : 1.4;
  const sectionTitleFont = fonts(spec).title;
  const sectionTitleSize = fitFontSize(sectionTitle, sectionTitleW, sectionTitleH, {
    fontName: sectionTitleFont, minSize: 16, maxSize: isQuarks ? 26 : 36, fillRatio: 0.80
  });
  slide.addText(
    sectionTitle,
    {
      ...
      fontFace: sectionTitleFont,      // Calibri for quarks, Cambria for others
      bold: true,
  ...
  if (spec.subtitle) {
    const sectionSubW = isQuarks ? 8.0 : 6;
    const sectionSubFont = fonts(spec).body;
    const sectionSubSize = fitFontSize(spec.subtitle, sectionSubW, 0.6, {
      fontName: sectionSubFont, minSize: 10, maxSize: isQuarks ? 14 : 16, fillRatio: 0.90
    });
    slide.addText(spec.subtitle, {
      x: 0.9,
      y: isQuarks ? 3.65 : 3.2,
...
async function renderBulletsWithIcons(slide, spec, pres) {
  ...
  const bwiTitleW = (spec.slideW || 13.33) - 1.0;
  const bwiTitleSize = fitFontSize(spec.title, bwiTitleW, 0.9, {
    fontName: 'Cambria', minSize: 14, maxSize: 22, fillRatio: 0.85
  });
  slide.addText(spec.title, {
    x: 0.5, y: 0.6, w: bwiTitleW, h: 0.9,
    fontSize: bwiTitleSize, fontFace: 'Cambria', bold: true,
    color: t.title_light, align: 'left', margin: 0,
    valign: 'top', fit: 'shrink'
  });

  if (spec.subtitle) {
    const bwiSubSize = fitFontSize(spec.subtitle, bwiTitleW, 0.35, {
      fontName: 'Calibri', minSize: 10, maxSize: 13, fillRatio: 0.90
    });
    slide.addText(spec.subtitle, {
      x: 0.5, y: 1.55, w: bwiTitleW, h: 0.35,
      fontSize: bwiSubSize, fontFace: 'Calibri', italic: true,
  ...
  const startY = spec.subtitle ? 2.0 : 1.6;
  ...
  for (let i = 0; i < count; i++) {
    ...
    // Bold first word + rest of text — combined into ONE text box with mixed runs
    // to prevent the two-box overlap that occurred when the bold word wrapped.
    // Dynamic font sizing: calculate optimal size to fit full text in the card.
    const words = bullets[i].split(' ');
    const boldWord = words[0];
    const rest = words.slice(1).join(' ');
    const fullText = rest ? boldWord + '\n' + rest : boldWord;
    const cardTextBoxW = cardW - 0.8;
    const cardTextBoxH = cardH - 0.3;
    const cardBaseSize = fitFontSize(fullText, cardTextBoxW, cardTextBoxH, {
      fontName: 'Calibri', minSize: 10, maxSize: 14, fillRatio: 0.88
    });
    const textRuns = [
      { text: boldWord, options: { bold: true, color: t.title_light, fontSize: cardBaseSize, breakLine: !!rest } },
    ];
    if (rest) {
      textRuns.push({ text: rest, options: { color: t.body_light, fontSize: Math.max(10, cardBaseSize - 1) } });
    }

    slide.addText(textRuns, {
      x: x + 0.65, y: y + 0.15, w: cardTextBoxW, h: cardTextBoxH,
...
async function renderSplitTwoCol(slide, spec, pres) {
  ...
  const stcTitleW = (spec.slideW || 13.33) - 1.0;
  const stcTitleSize = fitFontSize(spec.title, stcTitleW, 0.9, {
    fontName: 'Cambria', minSize: 14, maxSize: 22, fillRatio: 0.85
  });
  slide.addText(spec.title, {
    x: 0.5, y: 0.6, w: stcTitleW, h: 0.9,
    fontSize: stcTitleSize, fontFace: 'Cambria', bold: true,
    color: t.title_light, align: 'left', margin: 0,
    valign: 'top', fit: 'shrink'
  });

  // Dynamic two-column layout — scales with slideW instead of hardcoded positions
  ...
  if (bullets.length) {
    ...
    // Accent left bar
    slide.addShape(pres.shapes.RECTANGLE, {
      x: leftX, y: cardY, w: 0.06, h: cardH,
      fill: { color: t.primary },
      line: { color: t.primary, width: 0 }
    });

    // Dynamic font size for bullet list — fits all bullets in the card
    const stcBulletSize = fitBulletListFontSize(bullets, colW - 0.4, cardH - 0.3, {
      fontName: 'Calibri', minSize: 10, maxSize: 15, fillRatio: 0.90, paraSpaceAfter: 8
    });
    slide.addText(
      bullets.map((b, i) => ({
        text: b,
        options: { bullet: true, breakLine: i < bullets.length - 1, paraSpaceAfter: 8 }
      })),
      {
        x: leftX + 0.25, y: cardY + 0.15, w: colW - 0.4, h: cardH - 0.3,
        fontSize: stcBulletSize, fontFace: 'Calibri', color: t.body_light, valign: 'top',
        fit: 'shrink'
      }
    );
  }
  ...
  if (spec.stat_callout) {
    ...
    // Accent top bar
    slide.addShape(pres.shapes.RECTANGLE, {
      x: rightX, y: cardY - 0.2, w: colW, h: 0.06,
      fill: { color: t.accent },
      line: { color: t.accent, width: 0 }
    });

    // Dynamic stat number font size
    const stcStatSize = fitFontSize(spec.stat_callout.number, colW, 1.4, {
      fontName: 'Cambria', minSize: 24, maxSize: 56, fillRatio: 0.85
    });
    slide.addText(spec.stat_callout.number, {
      x: rightX, y: cardY + 0.45, w: colW, h: 1.4,
      fontSize: stcStatSize, fontFace: 'Cambria', bold: true,
      color: t.primary, align: 'center', valign: 'middle', margin: 0,
      fit: 'shrink'
    });
    // Dynamic stat label font size
    const stcLabelSize = fitFontSize(spec.stat_callout.label, colW, 0.5, {
      fontName: 'Calibri', minSize: 10, maxSize: 13, fillRatio: 0.90
    });
    slide.addText(spec.stat_callout.label, {
      x: rightX, y: cardY + 1.95, w: colW, h: 0.5,
      fontSize: stcLabelSize, fontFace: 'Calibri',
    ...
    slide.addShape(pres.shapes.RECTANGLE, {
      x: rightX, y: cardY, w: colW, h: 0.06,
      fill: { color: t.accent },
      line: { color: t.accent, width: 0 }
    });
...
async function renderComparison(slide, spec, pres) {
  ...
  const cmpTitleW = (spec.slideW || 13.33) - 1.0;
  const cmpTitleSize = fitFontSize(spec.title, cmpTitleW, 0.85, {
    fontName: 'Cambria', minSize: 14, maxSize: 22, fillRatio: 0.85
  });
  slide.addText(spec.title, {
    x: 0.5, y: 0.6, w: cmpTitleW, h: 0.85,
    fontSize: cmpTitleSize, fontFace: 'Cambria', bold: true,
    color: t.title_light, align: 'left', margin: 0,
    valign: 'top', fit: 'shrink'
  });
  ...
  // Dynamic font sizes for bullet lists — fit all content
  const cmpLeftSize = left.length ? fitBulletListFontSize(left, colW - 0.4, bodyH - 0.2, {
    fontName: 'Calibri', minSize: 10, maxSize: 14, fillRatio: 0.90, paraSpaceAfter: 8
  }) : 14;
  const cmpRightSize = right.length ? fitBulletListFontSize(right, colW - 0.4, bodyH - 0.2, {
    fontName: 'Calibri', minSize: 10, maxSize: 14, fillRatio: 0.90, paraSpaceAfter: 8
  }) : 14;

  // Left column
  slide.addShape(pres.shapes.RECTANGLE, {
    x: leftX, y: headerY, w: colW, h: headerH,
  ...
  if (left.length) {
    slide.addText(
      ...
      { x: leftX + 0.2, y: bodyY + 0.1, w: colW - 0.4, h: bodyH - 0.2, fontSize: cmpLeftSize, fontFace: 'Calibri', color: t.body_light, valign: 'top', fit: 'shrink' }
    );
  }

  // Right column
  slide.addShape(pres.shapes.RECTANGLE, {
    x: rightX, y: headerY, w: colW, h: headerH,
    fill: { color: t.primary },
    line: { color: t.primary, width: 0 }
  });
  ...
  if (right.length) {
    slide.addText(
      ...
      { x: rightX + 0.2, y: bodyY + 0.1, w: colW - 0.4, h: bodyH - 0.2, fontSize: cmpRightSize, fontFace: 'Calibri', color: t.body_light, valign: 'top', fit: 'shrink' }
    );
  }
...
async function renderDataChart(slide, spec, pres) {
  ...
  const dcTitleW = (spec.slideW || 13.33) - 4.0;
  const dcTitleSize = fitFontSize(spec.title, dcTitleW, 0.85, {
    fontName: 'Cambria', minSize: 14, maxSize: 22, fillRatio: 0.85
  });
  slide.addText(spec.title, {
    x: 0.5, y: 0.6, w: dcTitleW, h: 0.85,
    fontSize: dcTitleSize, fontFace: 'Cambria', bold: true,
    color: t.title_light, align: 'left', margin: 0,
    valign: 'top', fit: 'shrink'
  });
  ...
  if (spec.stat_callout) {
    const dcStatSize = fitFontSize(spec.stat_callout.number, 2.6, 0.65, {
      fontName: 'Cambria', minSize: 20, maxSize: 48, fillRatio: 0.85
    });
    slide.addText(spec.stat_callout.number, {
      x: 7.2, y: 0.35, w: 2.6, h: 0.65,
      fontSize: dcStatSize, fontFace: 'Cambria', bold: true,
      color: t.primary, align: 'right', valign: 'middle', margin: 0
    });
    const dcLabelSize = fitFontSize(spec.stat_callout.label, 2.6, 0.3, {
      fontName: 'Calibri', minSize: 10, maxSize: 11, fillRatio: 0.90
    });
    slide.addText(spec.stat_callout.label, {
      x: 7.2, y: 1.0, w: 2.6, h: 0.3,
      fontSize: dcLabelSize, fontFace: 'Calibri',
...
async function renderCaseStudy(slide, spec, pres) {
  ...
  const csTitleW = (spec.slideW || 13.33) - 1.0;
  const csTitleSize = fitFontSize(spec.title, csTitleW, 0.85, {
    fontName: 'Cambria', minSize: 14, maxSize: 22, fillRatio: 0.85
  });
  slide.addText(spec.title, {
    x: 0.5, y: 0.6, w: csTitleW, h: 0.85,
    fontSize: csTitleSize, fontFace: 'Cambria', bold: true,
    color: t.title_light, align: 'left', margin: 0,
    valign: 'top', fit: 'shrink'
  });
  ...
  slide.addText([
    { text: csQuoteText, options: { italic: true, color: t.title_light, fontSize: csQuoteSize } }
  ], {
  ...
  for (let i = 0; i < Math.min(bullets.length, 4); i++) {
    ...
    // Dynamic font size for result card text
    const csCardSize = fitFontSize(bullets[i], cardW - 0.72, 0.88, {
      fontName: 'Calibri', minSize: 10, maxSize: 13, fillRatio: 0.88
    });
    slide.addText(bullets[i], {
      x: x + 0.66, y, w: cardW - 0.72, h: 0.88,
      fontSize: csCardSize, fontFace: 'Calibri',
      color: t.body_light, valign: 'middle', margin: 0,
      fit: 'shrink'
    });
  }
...
async function renderCTA(slide, spec, pres) {
  ...
  // Line 1 — white title — dynamic font size to fit full title
  const ctaTitle = spec.title || `Let's Transform ${spec.client_name || 'Your Business'}`;
  const ctaTitleFont = fonts(spec).title;
  const ctaTitleSize = fitFontSize(ctaTitle, textW, 0.7, {
    fontName: ctaTitleFont, minSize: 16, maxSize: 28, fillRatio: 0.85
  });
  slide.addText(ctaTitle, {
    x: textX, y: 0.8, w: textW, h: 0.7,
    fontSize: ctaTitleSize, fontFace: ctaTitleFont, bold: true,
    color: t.title_dark,
  ...
  const subBoxH = subtitleLen > 100 ? 1.0 : subtitleLen > 60 ? 0.8 : 0.6;
  const ctaSubSize = fitFontSize(ctaSubtitle, textW, subBoxH, {
    fontName: ctaSubFont, minSize: 12, maxSize: 22, fillRatio: 0.88
  });
  slide.addText(ctaSubtitle, {
    x: textX, y: 1.6, w: textW, h: subBoxH,
    fontSize: ctaSubSize, fontFace: ctaSubFont, bold: true,
    color: t.secondary,
    align: 'center', valign: 'middle', margin: 0,
    fit: 'shrink'
  });
  ...
  const taglineY = 1.6 + subBoxH + 0.1;
  const ctaTaglineSize = fitFontSize(tagline, textW, 0.3, {
    fontName: fonts(spec).body, minSize: 10, maxSize: 14, fillRatio: 0.90
  });
  slide.addText(tagline, {
    x: textX, y: taglineY, w: textW, h: 0.3,
    fontSize: ctaTaglineSize, fontFace: fonts(spec).body,
    color: t.body_dark,
  ...
  const btnW = isQuarks ? 4.2 : 4.5;
  const gap = isQuarks ? 0.4 : 1.0;
  const startX = (slideW - (btnW * 2 + gap)) / 2;
  const btnY = taglineY + 0.35;

  // Button 1 — left
  slide.addShape(pres.shapes.RECTANGLE, {
    x: startX, y: btnY, w: btnW, h: 0.7,
    fill: { color: t.primary },
    line: { type: 'none' }
  });

  const btn1Text = (spec.brand?.website ? `Visit: www.${spec.brand.website.replace(/^https?:\/\/(www\.)?/, '')}` : spec.bullets?.[0]) || 'Visit: www.qtsolv.com';
  const ctaBtn1Size = fitFontSize(btn1Text, btnW, 0.7, {
    fontName: fonts(spec).title, minSize: 10, maxSize: 13, fillRatio: 0.88
  });
  slide.addText(btn1Text, {
    x: startX, y: btnY, w: btnW, h: 0.7,
    fontSize: ctaBtn1Size, fontFace: fonts(spec).title, bold: true,
    color: t.title_dark,
    align: 'center', valign: 'middle', margin: 0,
    fit: 'shrink'
  });
  ...
  // Button 2 — right
  slide.addShape(pres.shapes.RECTANGLE, {
    x: startX + btnW + gap, y: btnY, w: btnW, h: 0.7,
    fill: { color: t.primary },
    line: { type: 'none' }
  });

  const btn2Text = (spec.brand?.contact_email ? `Email: ${spec.brand.contact_email}` : spec.bullets?.[1]) || 'Email: contact@qtsolv.com';
  const ctaBtn2Size = fitFontSize(btn2Text, btnW, 0.7, {
    fontName: fonts(spec).title, minSize: 10, maxSize: 13, fillRatio: 0.88
  });
  slide.addText(btn2Text, {
    x: startX + btnW + gap, y: btnY, w: btnW, h: 0.7,
    fontSize: ctaBtn2Size, fontFace: fonts(spec).title, bold: true,
    color: t.title_dark,
    align: 'center', valign: 'middle', margin: 0,
    fit: 'shrink'
  });
  ...
  if (footer1) {
    const ctaFooterSize = fitFontSize(footer1, textW, 0.3, {
      fontName: fonts(spec).body, minSize: 10, maxSize: 12, fillRatio: 0.90
    });
    slide.addText(footer1, {
      x: textX, y: btnY + 0.85, w: textW, h: 0.3,
      fontSize: ctaFooterSize, fontFace: fonts(spec).body,
      color: t.body_dark,
...
async function renderAgenda(slide, spec, pres) {
  ...
  const agTitleW = (spec.slideW || 13.33) - 1.0;
  const agTitleSize = fitFontSize(spec.title, agTitleW, 0.85, {
    fontName: 'Cambria', minSize: 14, maxSize: 22, fillRatio: 0.85
  });
  slide.addText(spec.title, {
    x: 0.5, y: 0.6, w: agTitleW, h: 0.85,
    fontSize: agTitleSize, fontFace: 'Cambria', bold: true,
    color: t.title_light, align: 'left', margin: 0,
    valign: 'top', fit: 'shrink'
  });

  if (spec.subtitle) {
    const agSubSize = fitFontSize(spec.subtitle, agTitleW, 0.35, {
      fontName: 'Calibri', minSize: 10, maxSize: 13, fillRatio: 0.90
    });
    slide.addText(spec.subtitle, {
      x: 0.5, y: 1.5, w: agTitleW, h: 0.35,
      fontSize: agSubSize, fontFace: 'Calibri', italic: true,
      color: t.secondary, align: 'left', margin: 0,
      valign: 'top', fit: 'shrink'
    });
  }
  ...
  const totalW = (spec.slideW || 13.33) - 1.0;
  const gapX = 0.5;
  const colW = (totalW - gapX) / 2;
  const cardW = colW - 0.55;
  ...
  const drawNumberedItems = async (list, xStart, globalOffset) => {
    for (let idx = 0; idx < list.length; idx++) {
      ...
      // Card behind text
      slide.addShape(pres.shapes.RECTANGLE, {
        x: xStart + 0.55, y: y, w: cardW, h: 0.56,
        fill: { color: t.card_bg },
        shadow: makeShadow(0.04),
        line: { color: t.card_bg, width: 0 }
      });

      // Dynamic font size for agenda item text
      const agItemSize = fitFontSize(text, cardW - 0.3, 0.56, {
        fontName: 'Calibri', minSize: 10, maxSize: 14, fillRatio: 0.88
      });
      slide.addText(text, {
        x: xStart + 0.7, y, w: cardW - 0.3, h: 0.56,
        fontSize: agItemSize, fontFace: 'Calibri',
        color: t.body_light, valign: 'middle', margin: 0,
        fit: 'shrink'
      });
    }
  ...
  if (rightItems.length) await drawNumberedItems(rightItems, 0.5 + colW + gapX, col2Start);
...
async function renderPricing(slide, spec, pres) {
  ...
  const prTitleW = (spec.slideW || 13.33) - 1.0;
  const prTitleSize = fitFontSize(spec.title, prTitleW, 0.85, {
    fontName: 'Cambria', minSize: 14, maxSize: 22, fillRatio: 0.85
  });
  slide.addText(spec.title, {
    x: 0.5, y: 0.6, w: prTitleW, h: 0.85,
    fontSize: prTitleSize, fontFace: 'Cambria', bold: true,
    color: t.title_light, align: 'left', margin: 0,
    valign: 'top', fit: 'shrink'
  });

  if (spec.subtitle) {
    const prSubSize = fitFontSize(spec.subtitle, prTitleW, 0.35, {
      fontName: 'Calibri', minSize: 10, maxSize: 13, fillRatio: 0.90
    });
    slide.addText(spec.subtitle, {
      x: 0.5, y: 1.5, w: prTitleW, h: 0.35,
      fontSize: prSubSize, fontFace: 'Calibri', italic: true,
      color: t.secondary, align: 'left', margin: 0,
      valign: 'top', fit: 'shrink'
    });
  }
  ...
  if (rows.length) {
    ...
    const totalW = (spec.slideW || 13.33) - 1.0;
    const cw = [ (2.5/9)*totalW, (4.5/9)*totalW, (2/9)*totalW ];
    // Dynamic table font size — find the longest cell text to size appropriately
    const allCellTexts = tableRows.flat().map(c => c.text || '');
    const maxCellText = allCellTexts.reduce((a, b) => a.length > b.length ? a : b, '');
    const prTableSize = fitFontSize(maxCellText, (4.5/9)*totalW - 0.2, 0.50, {
      fontName: 'Calibri', minSize: 10, maxSize: 13, fillRatio: 0.85
    });
    slide.addTable(tableRows, {
      x: 0.5, y: 1.5, w: totalW,
      colW: cw,
      border: { pt: 0.5, color: 'E2E8F0' },
      fontFace: 'Calibri', fontSize: prTableSize,
      rowH: 0.50
    });
  } else if (spec.stat_callout) {
    const totalW = (spec.slideW || 13.33) - 1.0;
    const prStatSize = fitFontSize(spec.stat_callout.number, totalW, 1.5, {
      fontName: 'Cambria', minSize: 28, maxSize: 72, fillRatio: 0.85
    });
    slide.addText(spec.stat_callout.number, {
      x: 0.5, y: 2.3, w: totalW, h: 1.5,
      fontSize: prStatSize, fontFace: 'Cambria', bold: true,
      color: t.primary, align: 'center', valign: 'middle', margin: 0
    });
    const prLabelSize = fitFontSize(spec.stat_callout.label, prTitleW, 0.5, {
      fontName: 'Calibri', minSize: 10, maxSize: 14, fillRatio: 0.90
    });
    slide.addText(spec.stat_callout.label, {
      x: 0.5, y: 3.9, w: prTitleW, h: 0.5,
      fontSize: prLabelSize, fontFace: 'Calibri',
      color: t.body_light, align: 'center', margin: 0,
      fit: 'shrink'
    });
  }
...
async function renderCardsGrid(slide, spec, pres) {
  ...
  // ── Slide title — dynamic font size ──────────────────────────────
  const cgTitleW = isQuarks ? 9.0 : (spec.slideW || 13.33) - 1.0;
  const cgTitleH = isQuarks ? 0.85 : 0.9;
  const cgTitleFont = fonts(spec).title;
  const cgTitleSize = fitFontSize(spec.title, cgTitleW, cgTitleH, {
    fontName: cgTitleFont, minSize: 14, maxSize: isQuarks ? 26 : 26, fillRatio: 0.80
  });
  slide.addText(spec.title, {
    x: 0.5, y: isQuarks ? 0.6 : 0.5,
    w: cgTitleW,
    h: cgTitleH,
    fontSize: cgTitleSize,
    fontFace: cgTitleFont,        // Calibri for quarks, Cambria for others
    bold: true,
    color: t.title_light,
    align: 'left', margin: 0,
    valign: 'top', fit: 'shrink'
  ...
  if (spec.subtitle) {
    const cgSubFont = fonts(spec).body;
    const cgSubSize = fitFontSize(spec.subtitle, cgTitleW, 0.35, {
      fontName: cgSubFont, minSize: 10, maxSize: 13, fillRatio: 0.90
    });
    slide.addText(spec.subtitle, {
      x: 0.5, y: isQuarks ? 1.3 : 1.25,
      w: cgTitleW,
      h: 0.35,
      fontSize: cgSubSize,
      fontFace: cgSubFont,       // Calibri Light for quarks
      italic: true,
      color: t.secondary,
      align: 'left', margin: 0,
      valign: 'top', fit: 'shrink'
  ...
  // 3 columns max for ALL counts — 4 columns made cards too narrow
  // (2.12" wide → 1.36" text box) for text to fit without overflow
  let cols, rows;
  if (count <= 3) { cols = 3; rows = 1; }
  else if (count <= 4) { cols = 2; rows = 2; }
  else if (count <= 6) { cols = 3; rows = 2; }
  else if (count <= 9) { cols = 3; rows = 3; }
  else { cols = 3; rows = 4; }  // was 4×3 → 3×4 gives wider, taller cards

  // Dense grids (7+ items) need tighter spacing and more vertical room
  const isDense = count > 6;
  const gapX = isDense ? 0.15 : 0.18;
  const gapY = isDense ? 0.12 : 0.15;
  const totalW = isQuarks ? 9.0 : (spec.slideW || 13.33) - 1.0;
  const cardW = (totalW - (cols - 1) * gapX) / cols;
  // For dense grids, compress title area and extend to bottom of slide
  const denseStartY = spec.subtitle ? 1.55 : 1.35;
  const denseBottom = isQuarks ? 4.85 : 5.2;
  const availH = isDense
    ? denseBottom - denseStartY
    : (isQuarks ? 4.3 : 4.7) - startY;
  const cardH = (availH - (rows - 1) * gapY) / rows;
  const effectiveStartY = isDense ? denseStartY : startY;
  ...
  for (let i = 0; i < count; i++) {
    ...
    // ── Icon circle — smaller for dense grids ────────────────────────
    const iconSize = isDense ? (isQuarks ? 0.26 : 0.24) : (isQuarks ? 0.32 : 0.3);
    const iconX = isQuarks
      ? x + 0.15                          // left-aligned for quarks (left bar layout)
    ...
    // ── Card text — ALL layouts use a SINGLE combined text box ──────
    // This eliminates the overlap that occurred when title and desc were
    // in separate boxes and the title wrapped into the desc area.
    // Dynamic font sizing calculates the optimal size to fit ALL text.
    if (isDense) {
      // Dense grid: text sits below the icon, uses full card width
      const cgTextBoxW = cardW - 0.2;
      const cgTextBoxH = cardH - iconSize - 0.18;
      const cgFullText = desc ? title + '\n' + desc : title;
      const cgBaseSize = fitFontSize(cgFullText, cgTextBoxW, cgTextBoxH, {
        fontName: fonts(spec).body, minSize: 10, maxSize: 11, fillRatio: 0.90
      });
      const textRuns = [
        { text: title, options: { bold: true, color: t.title_light, fontSize: cgBaseSize, breakLine: !!desc } },
      ];
      if (desc) {
        textRuns.push({ text: desc, options: { color: t.body_light, fontSize: Math.max(10, cgBaseSize - 1) } });
      }
      slide.addText(textRuns, {
        x: x + 0.1, y: y + iconSize + 0.08,
        w: cgTextBoxW, h: cgTextBoxH,
        fontFace: fonts(spec).body,
        align: 'left', valign: 'top', margin: 0,
        fit: 'shrink'
      });
    } else if (isQuarks) {
      // Quarks: left-aligned text sitting to the right of left-bar + icon
      // Single combined text box prevents title/desc overlap
      const cgTextBoxW = cardW - 0.65;
      const cgTextBoxH = cardH - 0.16;
      const cgFullText = desc ? title + '\n' + desc : title;
      const cgBaseSize = fitFontSize(cgFullText, cgTextBoxW, cgTextBoxH, {
        fontName: fonts(spec).title, minSize: 10, maxSize: 12, fillRatio: 0.90
      });
      const textRuns = [
        { text: title, options: { bold: true, color: t.title_light, fontSize: cgBaseSize, breakLine: !!desc } },
      ];
      ...
      // Non-quarks: centered layout — single combined text box below icon
      const cgTextBoxW = cardW - 0.16;
      const cgTextBoxH = cardH - iconSize - 0.18;
      const cgFullText = desc ? title + '\n' + desc : title;
      const cgBaseSize = fitFontSize(cgFullText, cgTextBoxW, cgTextBoxH, {
        fontName: 'Calibri', minSize: 10, maxSize: 12, fillRatio: 0.90
      });
      const textRuns = [
        { text: title, options: { bold: true, color: t.title_light, fontSize: cgBaseSize, breakLine: !!desc } },
      ];
...
async function renderThreeColumn(slide, spec, pres) {
  ...
  const tcSlideW = spec.slideW || 13.33;
  const tcTitleW = tcSlideW - 1.0;
  const tcTitleSize = fitFontSize(spec.title, tcTitleW, 0.85, {
    fontName: 'Cambria', minSize: 14, maxSize: 26, fillRatio: 0.85
  });

  slide.addText(spec.title, {
    x: 0.5, y: 0.6, w: tcTitleW, h: 0.85,
    fontSize: tcTitleSize, fontFace: 'Cambria', bold: true,
    color: t.title_light, align: 'left', margin: 0,
    valign: 'top', fit: 'shrink'
  });

  if (spec.subtitle) {
    const tcSubSize = fitFontSize(spec.subtitle, tcTitleW, 0.35, {
      fontName: 'Calibri', minSize: 10, maxSize: 13, fillRatio: 0.90
    });
    slide.addText(spec.subtitle, {
      x: 0.5, y: 1.5, w: tcTitleW, h: 0.35,
      fontSize: tcSubSize, fontFace: 'Calibri', italic: true,
      color: t.secondary, align: 'left', margin: 0,
      valign: 'top', fit: 'shrink'
    });
  }
  ...
  for (let c = 0; c < Math.min(columns.length, 3); c++) {
    ...
    const colHeaderSize = fitFontSize(col.header, colW, headerH, {
      fontName: 'Calibri', minSize: 10, maxSize: 13, fillRatio: 0.90
    });
    slide.addText(col.header, {
      x, y: startY, w: colW, h: headerH,
      fontSize: colHeaderSize, fontFace: 'Calibri', bold: true,
    ...
    if (col.items && col.items.length) {
      const colBodySize = fitBulletListFontSize(col.items, colW - 0.24, bodyH - 0.2, {
        fontName: 'Calibri', minSize: 10, maxSize: 12, fillRatio: 0.90
      });
      slide.addText(
        ...
        })),
        {
          ...
          fontSize: colBodySize, fontFace: 'Calibri', color: t.body_light, valign: 'top',
          fit: 'shrink'
        }
      );
    }
...
async function renderClientLogos(slide, spec, pres) {
  ...
  const clSlideW = spec.slideW || 13.33;
  const clTitleW = clSlideW - 1.0;
  const clTitleSize = fitFontSize(spec.title, clTitleW, 0.85, {
    fontName: 'Cambria', minSize: 14, maxSize: 26, fillRatio: 0.85
  });

  slide.addText(spec.title, {
    x: 0.5, y: 0.6, w: clTitleW, h: 0.85,
    fontSize: clTitleSize, fontFace: 'Cambria', bold: true,
    color: t.title_light, align: 'left', margin: 0,
    valign: 'top', fit: 'shrink'
  });

  if (spec.subtitle) {
    const clSubSize = fitFontSize(spec.subtitle, clTitleW, 0.35, {
      fontName: 'Calibri', minSize: 10, maxSize: 13, fillRatio: 0.90
    });
    slide.addText(spec.subtitle, {
      x: 0.5, y: 1.5, w: clTitleW, h: 0.35,
      fontSize: clSubSize, fontFace: 'Calibri', italic: true,
      color: t.secondary, align: 'left', margin: 0,
      valign: 'top', fit: 'shrink'
    });
  }
  ...
  for (let i = 0; i < count; i++) {
    ...
    const badgeTextW = badgeW - 0.24;
    const badgeTextSize = fitFontSize(clients[i], badgeTextW, badgeH, {
      fontName: 'Calibri', minSize: 10, maxSize: 11, fillRatio: 0.90
    });
    slide.addText(clients[i], {
      x: x + 0.12, y, w: badgeTextW, h: badgeH,
      fontSize: badgeTextSize, fontFace: 'Calibri', bold: true,
...
async function build(deckSpec, outputPath) {
  ...
  for (const slideSpec of deckSpec.slides) {
    // Force DARK_MASTER for inherently dark layouts
    const isInherentlyDark = slideSpec.layout === 'section_header' || slideSpec.layout === 'cta_dark' || slideSpec.layout === 'cover';
    const masterName = (slideSpec.is_dark_slide || isInherentlyDark) ? 'DARK_MASTER' : 'LIGHT_MASTER';
    const slide = pres.addSlide({ masterName });

    const slideW = isQuarksTheme ? 10 : 13.33;
    const enriched = {
      ...slideSpec,
      is_dark_slide: slideSpec.is_dark_slide || isInherentlyDark,
      theme,
      brand,
      client_name: deckSpec.client_name,
      slideW,
      slideH: isQuarksTheme ? 5.625 : 7.5,
    };
