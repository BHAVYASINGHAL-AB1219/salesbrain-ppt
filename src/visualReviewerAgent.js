...
function generateStructuralFeedback(issues, slideSpec) {
  ...
  // Track the worst vertical overflow so we can size the character budget
  let worstOverflowPct = 0;
  let worstOverflowShape = null;

  for (const issue of issues) {
    switch (issue.type) {
      // ── Vertical overflow (text taller than its box) — the MAIN overflow case ──
      case 'text_vertical_overflow': {
        const pct = issue.overflow_percent || 0;
        if (pct > worstOverflowPct) {
          worstOverflowPct = pct;
          worstOverflowShape = issue;
        }
        feedback.push(
          `Text overflows its box vertically by ~${pct}% (rendered ${issue.rendered_height}" > box ${issue.box_height}" at ${issue.font_size}pt). ` +
          `Shorten the text drastically — cut it to roughly ${Math.max(20, Math.round(100 / (1 + pct / 100)))}% of current length.`
        );
        // Severe overflow → ask for a layout with more room
        if (pct > 30 && !layoutSuggestion) {
          layoutSuggestion = suggestAlternativeLayout(slideSpec, 'more_space');
        }
        break;
      }

      // ── Near-overflow (fills >90% of box) — tighten before it breaks ──
      case 'text_near_overflow':
        feedback.push(
          `Text fills ${issue.fill_percent}% of its box — almost no breathing room. Shorten it slightly.`
        );
        break;

      // ── Horizontal overflow (a single word wider than the box) ──
      case 'text_horizontal_overflow':
        feedback.push(
          `A word of ${issue.longest_word_chars} chars is wider than its ${issue.box_width || ''}" box. ` +
          `Use shorter words or break long terms with hyphens.`
        );
        break;

      // ── Legacy alias kept for safety (older extractors may emit this) ──
      case 'text_overflow':
        feedback.push('Text overflows its container. Shorten the text significantly.');
        if (!layoutSuggestion) layoutSuggestion = suggestAlternativeLayout(slideSpec, 'more_space');
        break;

      case 'text_overlap':
        feedback.push(`Text elements overlap (${issue.overlap_percent}% overlap). Reduce text length or number of items.`);
        if (!layoutSuggestion) layoutSuggestion = suggestAlternativeLayout(slideSpec, 'fewer_items');
        break;

      case 'font_too_small':
        feedback.push(`Font size ${issue.font_size}pt is too small to read. Reduce content so the font can be larger (min 11pt).`);
        break;

      case 'out_of_bounds_right':
      case 'out_of_bounds_bottom':
        // Only act on real content overshoot, not intentional decorative bleeds
        // (decorative OVAL/RECTANGLE shapes are named "Shape N" and have no text)
        if (issue.severity === 'error') {
          feedback.push(`Content extends ${issue.overshoot_inches}" beyond the slide edge. Shorten the text.`);
        }
        break;
      ...
      case 'slide_overcrowded':
        feedback.push('The slide is overcrowded — too many elements. Reduce the number of items.');
        if (!layoutSuggestion) layoutSuggestion = suggestAlternativeLayout(slideSpec, 'fewer_items');
        break;
    }
  }

  // If we measured a real overflow, append a concrete character budget so the
  // content agent has a hard target instead of a vague "shorten it".
  if (worstOverflowShape) {
    const boxW = worstOverflowShape.box_width || 0;
    const fontPt = worstOverflowShape.font_size || 12;
    // Rough chars-per-line estimate (avg char width ≈ 0.5 × font size in pts)
    const charsPerLine = boxW > 0 ? Math.round((boxW * 72) / (fontPt * 0.5)) : 40;
    // Available lines = box height / line height; cut by overflow ratio
    const boxH = worstOverflowShape.box_height || 0;
    const availLines = boxH > 0 ? Math.floor((boxH * 72) / (fontPt * 1.25)) : 4;
    const safeLines = Math.max(1, Math.floor(availLines * 0.85));
    const charBudget = charsPerLine * safeLines;
    feedback.push(`HARD LIMIT: keep total text under ~${charBudget} characters across ${safeLines} lines.`);
  }
...
function getContentConstraints(failure, slideSpec) {
  ...
  // Find the worst vertical overflow so we can scale the tightening
  let worstOverflowPct = 0;
  for (const issue of issues) {
    if (issue.type === 'text_vertical_overflow') {
      worstOverflowPct = Math.max(worstOverflowPct, issue.overflow_percent || 0);
    }
  }

  for (const issue of issues) {
    // ── Vertical overflow: the primary driver for "shorten everything" ──
    if (issue.type === 'text_vertical_overflow' || issue.type === 'text_overflow') {
      constraints.force_shorter = true;
      // The more it overflows, the harder we clamp
      const severity = worstOverflowPct > 50 ? 3 : worstOverflowPct > 25 ? 2 : 1;
      constraints.max_title_words = Math.max(4, 8 - severity * 2);
      constraints.max_subtitle_words = Math.max(6, 15 - severity * 3);
      constraints.max_bullets = Math.max(2, (slideSpec.bullets?.length || 4) - severity);
      constraints.max_bullet_words = Math.max(5, 12 - severity * 2);
    }
    // ── Near overflow: tighten mildly so it doesn't tip over next time ──
    if (issue.type === 'text_near_overflow') {
      constraints.force_shorter = true;
      constraints.max_bullet_words = Math.min(constraints.max_bullet_words, 10);
    }
    if (issue.type === 'text_horizontal_overflow') {
      constraints.max_bullet_words = Math.min(constraints.max_bullet_words, 8);
    }
    ...
    if (issue.type === 'slide_overcrowded') {
